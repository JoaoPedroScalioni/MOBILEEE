...
> **Stack:** Expo SDK 54 (React Native + Expo Router) · SQLite via `expo-sqlite` + Drizzle ORM · Supabase (Postgres, Auth, Storage) · `expo-camera` / `expo-image-picker` · `expo-location` · `react-native-maps` · `expo-secure-store` · `@react-native-community/netinfo`  
...
| **Plataformas-alvo e Workflow** | Android e iOS (com suporte Web para fallback e demonstração). Desenvolvido em Expo Managed Workflow / Dev Client (Expo SDK 54), garantindo compatibilidade com módulos nativos sem necessidade de eject precoce. |
